    ...
    ) : ICommandHandler<UserRegisterRequestDto, UserRegisterResponseDto>
{
    ...
    public async Task<UserRegisterResponseDto> HandleCommandAsync(UserRegisterRequestDto command, CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation("Registering user {Email} - Success", command.Email);
